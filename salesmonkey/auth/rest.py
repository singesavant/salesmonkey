...
from erpnext_client.schemas import ERPAddressSchema, ERPContactSchema
...
class CustomerPostalAddress(MethodResource):
    ...
    def post(self, **kwargs):
        ...
        try:
            ...
            erp_client.query(ERPAddress).update(
                name=existing_address["name"], data=existing_address
            )
            ...
            link = erp_client.query(ERPDynamicLink).create(
                data={
                    "parent": address["name"],
                    "parenttype": "Address",
                    "parentfield": "links",
            ...
            LOGGER.debug(
                "Address successfully linked to Customer <{0}>.".format(link["name"])
            )
...
class CustomerContact(MethodResource):
    ...
    def post(self, **kwargs):
        ...
        try:
            ...
            contact_phone = {"phone": kwargs["mobile_no"], "is_primary_mobile_no": 1}
            ...
            if len(existing_contact["phone_nos"]) > 0:
                ...
                contact_phone = erp_client.query(ERPContactPhone).create(
                    data=contact_phone_data
                )
                existing_contact["phone_nos"].append(contact_phone)

            # Trigger update !
            erp_client.query(ERPContact).update(
                name=existing_contact["name"], data=existing_contact
            )
            ...
            LOGGER.debug(
                "Contact successfully linked to Customer <{0}>.".format(link["name"])
            )
...
class AuthWith(MethodResource):
    ...
    def _get_or_create_erp_user_from_google(self, token):
        ...
        erp_user = None
        try:
            erp_user = erp_client.query(ERPUser).get(
                json["email"], fields='["first_name", "last_name"]'
            )
            ...
            # Compute gender
            try:
                gender = {"male": "Male", "female": "Female"}[user_info_json["gender"]]
            except KeyError:
                gender = "Other"

            erp_user = erp_client.query(ERPUser).create(
                data={
    ...
    def _get_or_create_contact_and_customer_for_user(self, aUser):
        ...
        try:
            contact = erp_client.query(ERPContact).first(
                filters=[["Contact", "user", "=", aUser.username]],
            ...
            contact = erp_client.query(ERPContact).create(
                data={
            ...
            contact_email = erp_client.query(ERPContactEmail).create(
                data=contact_email_data
            )
        ...
        try:
            ...
            customer = erp_client.query(ERPCustomer).create(
                data={
